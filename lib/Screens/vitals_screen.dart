...
import 'package:shared_preferences/shared_preferences.dart'; // 💡 必须引入
...
class _VitalsScreenState extends State<VitalsScreen> {
  ...
  bool isLoading = true;
  int selectedIndex = 0; 
  String currentPatientId = "20"; // 💡 动态存储当前 ID

  // --- Clinical data (保持原样作为基准) ---
  final double clinicalHR = 72.0;
  final String clinicalBP = "120/80";
  final String clinicalECG = "Borderline";
  ...
  Future<void> _loadData() async {
    ...
    // 3. 💡 动态过滤：根据 searchId 过滤数据
    final filteredData = rawData.where((item) => 
      item['patient_id'].toString() == searchId
    ).toList();
    ...
    setState(() {
      currentPatientId = searchId; // 更新当前显示的 ID
      stepSpots = sSpots;
      calorieSpots = cSpots;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        // 💡 标题改为动态 ID
        title: Text("P$currentPatientId Health Dashboard"),
              ...
              children: [
                ...
                Center(
                  child: SegmentedButton<int>(
                    segments: const [
                      ButtonSegment(value: 0, label: Text('Steps'), icon: Icon(Icons.directions_walk)),
                      ButtonSegment(value: 1, label: Text('Calories'), icon: Icon(Icons.local_fire_department)),
                      ButtonSegment(value: 2, label: Text('HR Check'), icon: Icon(Icons.favorite_border)),
                    ],
          ...
          children: [
            Row(
              children: const [
                ...
                SizedBox(width: 8),
                Text("Clinical Records (eHospital)", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              ],
            ),
            ...
            Row(
              ...
              children: [
                _clinicalInfo("ECG Status", clinicalECG, Colors.orange),
                _clinicalInfo("Baseline HR", "${clinicalHR.toInt()} BPM", Colors.green),
                _clinicalInfo("Blood Pressure", clinicalBP, Colors.blue),
              ],
            ),
          ],
        ),
  ...
  Widget _buildChartSection() {
    String title = ["Steps", "Calories", "Heart Rate"][selectedIndex];
          ...
          child: LineChart(
            LineChartData(
              ...
              lineBarsData: [
                if (selectedIndex == 2)
                  LineChartBarData(
                    spots: [FlSpot(0, clinicalHR), FlSpot(timeLabels.isEmpty ? 0 : (timeLabels.length - 1).toDouble(), clinicalHR)],
    ...
    return Container(
      ...
      child: Row(
        children: const [
          ...
          Expanded(
            child: Text(
              "Sync Discrepancy Found: Active records found but wearable Heart Rate is 0 BPM.",
